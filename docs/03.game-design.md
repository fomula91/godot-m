...
> **상태**: 🟡 부분 갱신 필요 — 2.1 시놉시스(충돌 C-4), 2.3 수아 표정 종류(충돌 C-9)
> **인물 정보 정본**: [02.characters.md](02.characters.md) — 아래 2.3은 요약이며, 상세는 02를 따른다.
> **원고 정본**: [01.new_story.md](01.new_story.md) · **미결 추적**: [05.continuity.md](05.continuity.md)
...
| **시간 경과 페널티** | **허브 복귀마다 distance +10** |

플레이어의 선택에 따라 거리감이 증감하며, 수아와의 심리적 거리를 수치로 표현한다.
**아무 선택도 하지 않으면 거리감은 자연히 벌어진다** — 허브를 한 번 돌 때마다 +10이 가산되므로, 플레이어의 과제는 "관계를 쌓는 것"이 아니라 **"흘러가는 시간을 거스르는 것"** 이다.

#### 거리감 변화표

**허브 이벤트 (4종)** — 1차 선택지에서만 변동한다.
...
**선형 도입부로 이관** — 자리배정·수학시간은 2026-08-02 결정으로 허브에서 빠졌다. 도입부에서 거리감을 변동시킬지는 미결(→ `05.continuity.md` O-8).

| 이벤트 | 기존 선택지 | 기존 변화량 |
|--------|-----------|-----------|
| ~~자리배정~~ | 인사 / 친구대화 / 취미질문 | -5 / +15 / -15 |
| ~~수학시간~~ | 가르쳐줌 / 힌트만 | -15 / -5 |

> **디자인 의도**: "보지 않았다"가 가까워지는 이유는 억지로 다가가지 않는 것이 수아에게 부담을 주지 않기 때문이다. 반면 "역할을 묻는다"·"힐끗 봤다"는 수아가 자신과 다른 세계의 사람이라고 느끼게 만든다.
> **밸런스 검산은 `06.hub-structure.md` 8-1절**에 있다. 최선 플레이 45 / 최악 85(게임 오버)로 설계되어 있다.
...
| `seen_group` | int | 0 | 조별수업 이벤트 관람 여부 |
| `seen_lunch` | int | 0 | 점심시간 이벤트 관람 여부 |
| `seen_trip` | int | 0 | 수학여행 이벤트 관람 여부 |
| `seen_none` | int | 0 | "보지 않았다" 관람 여부 |
| `topic_*` | int | 0 | 화제 플래그 (H-4, 원고 4~6화와 함께 도입) |

`seen_*`은 **이벤트 중복 선택 방지**(H-1)에 쓴다. 정의되지 않은 변수는 엔진이 0으로 평가하므로 별도 초기화가 필요 없다.
명명 규약과 사용법은 `07.hub-implementation.md` 6절 참조.
...
Start (프롤로그)
  └─ Chapter1 (첫 합석)
       └─ SeatAssignment (자리배정 — 원고 2화)
            └─ MathClass (수학시간 — 원고 3화)     ※ 여기까지 선형 도입부
                 │
                 └─ AprilHub (4월 허브 / 이벤트 선택)
                      ├─ GroupProject (조별수업) ──┐
                      ├─ LunchTime (점심시간) ─────┤ → [조건 분기]
                      ├─ SchoolTrip (수학여행) ────┤
                      └─ DidNotLook (보지 않았다) ─┘
                                      │
              ┌────────────────────────┘
              │  (이벤트 종료 시 distance +10)
              ├─ distance >= 80 ────────→ GameOver
              ├─ april_events >= 3 ────→ CrackIntro (균열 파트)
              └─ else ─────────────────→ AprilHub (반복, 관람한 이벤트는 숨김)
...
4월 허브에서 플레이어는 **4가지 이벤트 중 하나**를 선택한다. 각 이벤트는 2단계 선택지로 구성되어 있다.
...
이벤트 종료마다 `april_events`가 +1, `distance`가 +10 되며, **3회 이상** 완료 시 균열 파트로 진행한다.
한 번 관람한 이벤트는 다음 진입부터 목록에서 숨겨지므로(`seen_*` 플래그), 4개 중 **하나는 반드시 보지 않게 된다.**

> **선형 도입부로 이관됨** (2026-08-02): 자리배정·수학시간은 허브 이벤트 조건을 만족하지 못해 도입부로 옮겼다. 자리배정은 되돌릴 수 없는 사건이고, 수학시간은 "옆자리"를 전제하기 때문이다. 판정 기준은 `06.hub-structure.md` 6절 참조.
...
| 엔딩 | 최종 선택 | 결과 | 톤 |
|------|----------|------|-----|
| **엔딩 A** — "있잖아…!" | "말 없이 멀어지지 않을게" **+ `distance <= 45`** | 수아가 먼저 약속을 잡고, 채원에게도 다시 연락한다. 가장 긍정적인 결말. | 희망적, 따뜻함 |
...
> **엔딩 A는 조건부다** (2026-08-02 확정): *"말 없이 멀어지지 않을게"* 를 골라도 **거리감이 45를 넘으면 엔딩 B로 간다.** 같은 말이라도 쌓아온 시간에 따라 다르게 닿는다는 작품 주제의 구현이며, 4월의 모든 선택에 소급해서 의미를 부여한다. 최선 플레이의 도달치가 정확히 45이므로 엔딩 A는 **완벽한 4월을 보낸 플레이어에게만** 열린다.
