# 04. 엔진 스크립트 (JSON 변환 원본)

> **상태**: 🔴 **구원고 기준.** 새 원고 [01.new_story.md](01.new_story.md)가 반영되지 않았다.
> 프롤로그~3화 구간은 새 원고로 재작성해야 하며, 동현(신규 조연)·자리배정 편입·대사 교체가 미반영 상태다. 갱신 범위는 [05.continuity.md](05.continuity.md) 4절 참조.
> **⚠️ 밸런스 수치도 구버전이다** (2026-08-02 ADR-0004): 이 문서 곳곳의 `april_events >= 4`는 **3으로**, "보지 않았다" `-15`는 **-5로** 바뀌었고, 이벤트 종료 시 `distance +10`(시간 경과 페널티)이 추가되었다. 자리배정·수학시간은 허브 이벤트가 아니라 **선형 도입부**로 이관되었다.
> **확정 수치의 정본은 [06.hub-structure.md](06.hub-structure.md) 8-1절**이며, 이 문서의 수치는 재작성 시 일괄 교체 대상이다.

> **유효한 부분**: 표기법·레이블 체계·부록(변수/갤러리 목록)은 게임 구조 유지 결정에 따라 그대로 살아 있다.
