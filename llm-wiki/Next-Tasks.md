...
**상태** — 코드는 반영됨(`choice_controller.gd`). **Godot 에디터 미실행으로 검증을 못 했다.** 에디터를 켜고 stop→run 후 아래 완료 기준을 확인하면 종료 가능. 같은 커밋에 Phase 0 엔진 정비(동현 개명·엑스트라 6종 등록·수아 표정 재매핑·`test` 디렉터리 로드)도 함께 들어갔으므로 그것도 같이 확인할 것.
